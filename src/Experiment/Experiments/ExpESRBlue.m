classdef ExpESRBlue < Experiment
    %EXPESRBlue ESR with Blue Laser (electron spin resonance) Experiment
    ...
    methods
        ...
        function obj = ExpESRBlue(FGs, MWChannel)
            obj@Experiment(ExpESRBlue.NAME);
            ...
            switch obj.mode
                case 'CW'
                    ...
                    switch obj.nChannels
                        case 1
                            P = Pulse(obj.detectionDuration,        {MWChannel, 'blueLaser', 'detector'});
                        case 2
                            P = Pulse(obj.detectionDuration,        {MWChannel, 'MW2', 'blueLaser', 'detector'});
                        otherwise
                            obj.sendError('What should we do here?')
                    end
                    
                    S.addEvent(obj.laserInitializationDuration,     {MWChannel, 'blueLaser'});
                    S.addPulse(P);
                    S.addEvent(obj.laserInitializationDuration,     {'blueLaser'});
                    S.addEvent(obj.referenceDetectionDuration,      {'blueLaser','detector'})
                case 'pulsed'
                    if obj.laserInitializationDuration < obj.detectionDuration
                    ...
                    S.addEvent(0.3,                       '');                        % Last delay
                    S.addEvent(obj.detectionDuration,           {'blueLaser','detector'})  % Detection
                    S.addEvent(initDuration,                    'greenLaser')               % Initialization
                    S.addEvent(0.3,                       '');                        % Last delay
                    S.addEvent(obj.referenceDetectionDuration,...
                                                                {'blueLaser','detector'})  % Reference detection
                    S.addEvent(initDuration,                    'greenLaser')               % Initialization
            end
