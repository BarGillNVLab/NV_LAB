classdef ExpDEER < Experiment
    ...
    properties
        ...
        mw_scan             % MW channel
        frequency_scan      % in MHz
        piTime_scan         % in us
        mw_chan_idx         % integer (or long)
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    methods
        function obj = ExpDEER(FG, MWChannel)
            obj@Experiment(ExpDEER.NAME);
            ...
            obj.mw_scan = [];
            obj.frequency_scan = 2870;      % in MHz
            obj.piTime_scan = 0.05;         % in us
...
%             obj.topParam = ExpParamDoubleVector(StringHelper.TAU, [], [], StringHelper.MICROSEC, obj.NAME);
            obj.topParam = ExpParamDoubleVector('Frequency', [], [], 'MHz', obj.NAME);
    ...
    methods
        ...
        function set.frequency_scan(obj, newVal) % newVal is in MHz
            checkFrequencyVector(obj, newVal)
            % If we got here, then newVal is OK.
            obj.frequency_scan = newVal;
            obj.changeFlag = true;
        end
        ...
        function set.piTime_scan(obj, newVal)
            checkTimeScalar(obj, newVal)
            % If we got here, then newVal is OK.
            obj.piTime_scan = newVal;
            obj.changeFlag = true;
        end

        function set.mw_chan_idx(obj, newVal)	% newVal in microsec
            checkFrequencyVector(obj, newVal)
            % If we got here, then newVal is OK.
            obj.mw_chan_idx = newVal;
            obj.changeFlag = true;
        end
    end
    ...
    methods
        function totalParamNum = getTotalNumberOfParams(obj)
            totalParamNum = length(obj.frequency_scan);
        end
    end
    ...
    methods (Access = protected)
        function prepare(obj)
            ...
            piTime_diff = abs(obj.piTime_scan - obj.piTime); %assuming piTime_scan is longer than piTime (relevant in the sequence)
            ...
            for k = 1:1+obj.doubleMeasurement
                ...
                if obj.useIQ %Pi Pulse-IQ
                    S.addEvent(piTime_diff/2,               obj.mw_scan,                    'scanning MW');
                    S.addEvent(obj.piTime,                  {'I', MWChannel, obj.mw_scan},  'piTime');                  % MW in y
                    S.addEvent(piTime_diff/2,               obj.mw_scan,                    'scanning MW'); % MW scanning the probed spin
                else
                    S.addEvent(piTime_diff/2,               obj.mw_scan,                    'scanning MW');
                    S.addEvent(obj.piTime,                  {MWChannel, obj.mw_scan},       'piTime');                         % MW in x
                    S.addEvent(piTime_diff/2,                  obj.mw_scan,               'scanning MW'); % MW scanning the probed spin
                end
            ...
            obj.mCurrentXAxisParam.value = obj.frequency_scan;
            obj.topParam.value = obj.frequency_scan;
        ...
        function perform(obj)
            ...
            %%% Run - Go over all frequencies's, in random order
            for t = randperm(length(obj.frequency_scan))
...
%                             pg.changeSequence('lastDelay', 'duration', maxLastDelay - 2*obj.tau(t));
                        obj.frequency(obj.mw_chan_idx) = obj.frequency_scan(t);
...
%                     end
            ...
            if length(data) == obj.detectionPeriodsPerRepeat*obj.repeats*obj.getTotalNumberOfParams % if true we're proccessing data at the end of the average
                for k = 1:length(obj.frequency_scan)
