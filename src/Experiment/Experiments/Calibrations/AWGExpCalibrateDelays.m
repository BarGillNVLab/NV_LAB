classdef AWGExpCalibrateDelays < ExperimentAWG
    %ExpCalibrateDelays Experiment for calibrating the delay times
    
    properties (Constant)
        NAME = 'AWG Calibration Delays';
    end
    ...
    properties
        ...
        channle                     % 1 or 2 - for choosing the AWG channle for calibration
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    methods
        function obj = AWGExpCalibrateDelays(FG)
    ...
    methods (Access = protected)        
        ...
        function LoadAWG(obj)
            MWduration = 0.8*obj.tEnd;
            segmentTime = round(obj.frequency*0.5) / obj.frequency;
            numSegments = ceil(MWduration/segmentTime) - 1;
            lastSegmentTime = MWduration - segmentTime*(numSegments-1);
            
            obj.AWG.ResetStoredData;
            obj.AWG.workChannels = 1;
            obj.AWG.ChangeSampleRate(segmentTime);
            ...
            f = @(t) A * (0<=t & t<lastSegmentTime) .* cos(w*t);
            mainWaveformsIndex = obj.AWG.AddWaveformByFunction(f, segmentTime, 0);
            lastWaveformsIndex = obj.AWG.AddWaveformByFunction(f, lastSegmentTime, 0, 1);
            f = @(t) 0*t;
            zeroIndex = obj.AWG.AddWaveformByFunction(f, minWaveformTime, 0, 1);
            obj.AWG.LoadWaveform();
            
            % creating sequences:
            obj.indexSeq = obj.AWG.AddSequence([zeroIndex mainWaveformsIndex lastWaveformsIndex zeroIndex], ...
                                               [1 numSegments-1 1 1], [1 0 0 0]);
            ...
            for n = randperm(length(obj.privateSequences))
                ...
                for trial = 1 : 5
                    ...
                    try
                        ...
                        obj.AWG.assignSequence(obj.indexSeq, 1)
                        obj.AWG.Run;
