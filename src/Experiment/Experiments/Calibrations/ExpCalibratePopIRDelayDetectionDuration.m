classdef ExpCalibratePopIRDelayDetectionDuration < Experiment
    % ExpSingletEnergy an experiment desined to find whether there is
    % excitation from the ground singlet state to the excited triplet. 
    % This experiment works only with setups that allow two laser powers 
    % (2 NIDAQ channels for the AOM for example).
    
    properties (Constant)
        NAME = 'CalibratePopIRDelayDetectionDuration';
    end
    ...
    properties
        ...
        initializationLaserPower        % in ??. A vector of laser powers.
        populationLaserPower        % in ??. A vector of laser powers.
    end
    
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    methods
        function obj = ExpCalibratePopIRDelayDetectionDuration(FG, MWChannel)
            obj@Experiment(ExpCalibratePopIRDelayDetectionDuration.NAME);
            ...
            obj.initializationLaserPower = 0.01; % in whatever units the setup works with
            obj.populationLaserPower = 0.3; % in whatever units the setup works with
            
            obj.constantTime = 0;
            obj.doubleMeasurement = true;   % logical
            
            obj.detectionDurations = [0.25,1]; % detection duration in us
            ...
            obj.waitTimesExcited = [0.1,1];
            obj.PopDuration = 0.4; %0.1;
...
%             laserPart = laserParts{1};
            ...
            obj.mCurrentXAxisParam = ExpParamDoubleVector('Population to Detection Delay', [], [], StringHelper.MICROSEC, obj.NAME);
            obj.mCurrentYAxisParam = ExpParamDoubleVector('IR Detection Duration', [], [], StringHelper.MICROSEC, obj.NAME);
%             obj.signalParam = ExpResultDoubleVector('FL', [], [], 'Normalized', obj.NAME);
    ...
    methods
        ...
        function set.detectionDurations(obj, newVal)
        ...
        function set.waitTimesExcited(obj, newVal)
            checkTimeVector(obj, newVal)
            % If we got here, then newVal is OK.
            obj.waitTimesExcited = newVal;
            obj.changeFlag = true;
        end
    ...
    methods
        function totalParamNum = getTotalNumberOfParams(obj)
            totalParamNum = length(obj.waitTimesExcited)*length(obj.detectionDurations);
        end
    end
    ...
    methods (Access = protected)
       function prepare(obj)
            ...
            initDuration = obj.laserInitializationDuration-obj.referenceDetectionDuration;
            ...
            for k = 1:1+obj.doubleMeasurement
                ...
                S.addEvent(0.3,          '')                                 % Delay for singlet state decay
                ...
                if k==1
                    ...
                    S.addEvent(obj.piTime,          '')                                 % Delay for excited state decay
                end
                %             S.addEvent(lastDelay,           '',                         'lastDelay');       % Last delay
                
                
                
%                 S.addEvent(max(obj.popDurations),              {'greenLaser','greenLaserOneOrTwo'} ,'popDuration')                       % Populating the singlet
%                 S.addEvent(obj.waitTimeExcited,          '')                                 % Delay for excited state decay

                %                 if k==1
                S.addEvent(obj.PopDuration,              {'greenLaser','greenLaserOneOrTwo'} )
                S.addEvent(max(obj.waitTimesExcited),              '' ,'waitTimeExcited')
%                 S.addEvent(obj.detectionDuration,          {'greenLaser','IR','detector'} ,'detctionDuration1')                  % Ionization from Singlet state
                S.addEvent(max(obj.detectionDurations),          {'IR','detector'}, 'detectionDuration')                  % Ionization from Singlet state
%                 S.addEvent(0.5,          {'IR'})
                %                 else
%                     S.addEvent(obj.detectionDuration,          {'detector'})                  % Ionization from Singlet state
%                 end
                
                S.addEvent(obj.waitTimeSinglet,          '')                                 % Delay for singlet state decay
...
%                 S.addEvent(0.5,          {'IR'})
            ...
            obj.mCurrentXAxisParam.value = obj.waitTimesExcited;
        ...
        function perform(obj)
            ...
            % Now assumes laser changing the first laser part:
            laserPart = laserParts{1}; 
            laserPart.aomOne.value = obj.initializationLaserPower; % change value of AOM1
            laserPart.aomTwo.value = obj.populationLaserPower; % change value of AOM1
...
%                 laserPart.aomTwo.value = obj.laserPowers(t); %change value of AOM2
            ...
            for p = randperm(length(obj.waitTimesExcited))
                for d = randperm(length(obj.detectionDurations))
                    ...
                    excitedWaitTime = obj.waitTimesExcited(p);
                    detectionTime = obj.detectionDurations(d);
                    %                     detctionDuration1 = popDuration;
                    %                     detctionDuration2 = obj.detectionDuration-detctionDuration1;
                    ...
                    for trial = 1 : 10
                        ...
                        try
                            ...
                            pg.changeSequence('waitTimeExcited' , 'Duration', excitedWaitTime)
                            pg.changeSequence('detectionDuration' , 'Duration', detectionTime)
                            %                             pg.changeSequence('detctionDuration1' , 'Duration', detctionDuration1)
                            %                             pg.changeSequence('detctionDuration2' , 'Duration', detctionDuration2)
                            %
...
%                             obj.sterr(:, p, obj.currIter) = sterr;
            ...
            value = reshape(value, length(obj.detectionDurations), length(obj.waitTimesExcited));
            sterr = reshape(sterr, length(obj.detectionDurations), length(obj.waitTimesExcited));
