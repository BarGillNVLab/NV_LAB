classdef ExpCalibrateSingletEnergyParams < Experiment
    ...
    properties (Constant)
        NAME = 'CalibrateSingletEnergyParams';
    end
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    methods
        function obj = ExpCalibrateSingletEnergyParams(FG, MWChannel)
            obj@Experiment(ExpCalibrateSingletEnergyParams.NAME);
            ...
            obj.laserInitializationDuration = 20; % laser initialization in pulsed experiments
            
            obj.PopDuration = 0.4; %1;
            obj.ionDuration = 0.1; %0.5; %1;
            obj.waitTimeExcited = 0.15; %0.03;
            obj.waitTimeSinglet = 10; %5; %0.5;
            
            obj.calibParam = 'ionDuration';
            obj.calibValues = [0.05:0.05:0.5];
...
%             obj.isPlotAlternateAvailable = true;
            ...
            obj.mCurrentXAxisParam = ExpParamDoubleVector('Ionization Duration', [], [], StringHelper.MICROSEC, obj.NAME);
%             obj.signalParam = ExpResultDoubleVector('FL', [], [], 'Normalized', obj.NAME);
            obj.signalParam = ExpResultDoubleVector('FL', [], [], 'Normalized', obj.NAME, '\pi');
            obj.signalParam2 = ExpResultDoubleVector('FL', [], [], 'Normalized', obj.NAME, 'no \pi');
        end
    end
    ...
    methods
        ...
        function set.calibValues(obj, newVal)
            checkTimeVector(obj, newVal)
            % If we got here, then newVal is OK.
            obj.calibValues = newVal;
            obj.changeFlag = true;
        end
        ...
        function set.ionLaserPower(obj ,newVal) %check if this works well
            checkGreenLaserPower(obj, newVal)
            % If we got here, then newVal is OK.
            obj.ionLaserPower = newVal;
            obj.changeFlag = true;
        end
        ...
        function set.calibParam(obj,newVal)
            switch newVal
                case 'ionDuration'
                    obj.calibParam = 'ionDuration';
                    obj.mCurrentXAxisParam = ExpParamDoubleVector('Ionization Duration', [], [], StringHelper.MICROSEC, obj.NAME);
                    obj.changeFlag = true;
                case 'waitTimeExcited'
                    obj.calibParam = 'waitTimeExcited';
                    obj.mCurrentXAxisParam = ExpParamDoubleVector('Population-Ionization Delay', [], [], StringHelper.MICROSEC, obj.NAME);
                    obj.changeFlag = true;
                case 'waitTimeSinglet'
                    obj.calibParam = 'waitTimeSinglet';
                    obj.mCurrentXAxisParam = ExpParamDoubleVector('Ionization-Readout Delay', [], [], StringHelper.MICROSEC, obj.NAME);
                    obj.changeFlag = true;
                otherwise
                    obj.sendError('Unknown mode! Ignoring.')
            end
    ...
    methods
        function totalParamNum = getTotalNumberOfParams(obj)
            totalParamNum = length(obj.calibValues);
        end
    end
       ...
       function prepare(obj)
            ...
            for k = 1:1+obj.doubleMeasurement
                for j = 1:(1+obj.referenceSpin)
                    ...
                    %                 S.addEvent(obj.PopDuration,              {'detector'} ,'populationDuration')
                    S.addEvent(obj.waitTimeExcited,          '','waitTimeExcited')                                 % Delay for excited state decay
                    if k==1
                        switch obj.laserSource
                            case 'White Laser'
                                %                     S.addEvent(obj.ionDuration,          {'detector'}, 'ionDuration' )
                    ...
                    S.addEvent(obj.waitTimeSinglet,          '','waitTimeSinglet')                                 % Delay for excited state decay
                    ...
                    %                 S.addEvent(obj.referenceDetectionDuration,{'detector'})
                    %                 S.addEvent(lastDelay,           '',                         'lastDelay');       % Last delay
                    S.addEvent(0.5,           '');
                end
            end
            ...
            obj.mCurrentXAxisParam.value = obj.calibValues;
            ...
            for d = randperm(length(obj.calibValues))
                ...
                for trial = 1 : 10
                    ...
                    try
                        ...
                        switch obj.calibParam
                            case 'ionDuration'
                                pg.changeSequence('ionDuration' , 'Duration', calibTime)
                            case 'waitTimeExcited'
                                pg.changeSequence('waitTimeExcited' , 'Duration', calibTime)
                            case 'waitTimeSinglet'
                                pg.changeSequence('waitTimeSinglet' , 'Duration', calibTime)
                            otherwise
                                obj.sendError('Unknown mode! Ignoring.')
                        end
                        ...
                        obj.signal(:, d, obj.currIter) = sig;
                        obj.sterr(:, d, obj.currIter) = sterr;
