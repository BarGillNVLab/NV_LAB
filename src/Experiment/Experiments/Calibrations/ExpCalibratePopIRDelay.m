classdef ExpCalibratePopIRDelay < Experiment
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    methods
        function obj = ExpCalibratePopIRDelay(FG, MWChannel)
            obj@Experiment(ExpCalibratePopIRDelay.NAME);
...
%             obj.isPlotAlternateAvailable = true;
            ...
            obj.mCurrentXAxisParam = ExpParamDoubleVector('Population Pulse Duration', [], [], StringHelper.MICROSEC, obj.NAME);
%             obj.signalParam = ExpResultDoubleVector('FL', [], [], 'Normalized', obj.NAME);
    ...
    methods
        ...
        function set.popDurations(obj, newVal)
            checkTimeVector(obj, newVal)
            % If we got here, then newVal is OK.
            obj.popDurations = newVal;
            obj.changeFlag = true;
        end
    ...
    methods
        function totalParamNum = getTotalNumberOfParams(obj)
            totalParamNum = length(obj.popDurations);
        end
    end
...
%                 S.addEvent(obj.waitTimeExcited,          '')                                 % Delay for excited state decay
                ...
                %                 if k==1
                S.addEvent(obj.PopDuration,              {'greenLaser'} )
                S.addEvent(max(obj.popDurations),              '' ,'popDuration')
%                 S.addEvent(obj.detectionDuration,          {'greenLaser','IR','detector'} ,'detctionDuration1')                  % Ionization from Singlet state
                S.addEvent(obj.detectionDuration,          {'IR','detector'}, 'detctionDuration2')                  % Ionization from Singlet state
                S.addEvent(0.5,          {'IR'})
                %                 else
...
%                 S.addEvent(obj.waitTimeExcited,          '')                                 % Delay for excited state decay
                ...
                S.addEvent(obj.referenceDetectionDuration,          {'IR','detector'})                  % Ionization from Singlet state
                S.addEvent(0.5,          {'IR'})
                S.addEvent(lastDelay,           '',                         'lastDelay');       % Last delay
            end
            ...
            obj.mCurrentXAxisParam.value = obj.popDurations;
...
%                 laserPart.aomTwo.value = obj.laserPowers(t); %change value of AOM2
                ...
                for p = randperm(length(obj.popDurations))
                    if obj.checkEmergencyStop()
                    return;
                    end
                    ...
                    popDuration = obj.popDurations(p);
...
%                     detctionDuration2 = obj.detectionDuration-detctionDuration1;
                    ...
                    for trial = 1 : 10
                        ...
                        try
                            ...
                            pg.changeSequence('popDuration' , 'Duration', popDuration)
...
%                             obj.signal(:, (t-1)*length(obj.popDurations)+p, obj.currIter) = sig;
%                             obj.sterr(:, (t-1)*length(obj.popDurations)+p, obj.currIter) = sterr;
