classdef GIA_GIgate_ExpCalibrateDelays < ExperimentAWG
    % calibration experiment of the delays between GIgate to GIdetector
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    methods
        function obj = GIA_GIgate_ExpCalibrateDelays(FG)
            ...
            obj.listOfChannels = {'GIgate', 'GIdetector'};
            obj.delays = 0:0.1:1.5;
            obj.otherDelays = 1;
            obj.centerDuration = 1;
            obj.onDelayMeas = 1;
    ...
    methods (Access = protected)        
        function prepare(obj)
            ...
            S = Sequence;
            S.addEvent(2,                               {'greenLaser', 'GIenable'});
            if obj.onDelayMeas
                S.addEvent(max(obj.delays),             {'greenLaser', obj.listOfChannels{1}, 'GIenable'}, 'delay1');
                S.addEvent(0,                           {'greenLaser', obj.listOfChannels{2}, 'GIenable'}, 'delay2');
                S.addEvent(abs(min(obj.delays)),        [{'greenLaser'}, obj.listOfChannels(:)', {'GIenable'}], 'delayBoth');
                S.addEvent(obj.centerDuration - abs(min(obj.delays)), ...
                                                        [{'greenLaser'}, obj.listOfChannels(:)', {'GIenable'}]);
                S.addEvent(obj.otherDelays,             {'greenLaser', obj.listOfChannels{1}, 'GIenable'});
            else
                S.addEvent(obj.otherDelays,             {'greenLaser', obj.listOfChannels{1}, 'GIenable'});
                S.addEvent(obj.centerDuration - abs(min(obj.delays)), ...
                                                        [{'greenLaser'}, obj.listOfChannels(:)', {'GIenable'}]);
            ...
            S.addEvent(2,                           {'greenLaser', 'GIenable'});
            S.addEvent(obj.acquisitionDuration,     {'greenLaser', 'detector', 'GIenable'});                % Acquisition
            S.addEvent(2,                           {'greenLaser', 'GIenable'});
