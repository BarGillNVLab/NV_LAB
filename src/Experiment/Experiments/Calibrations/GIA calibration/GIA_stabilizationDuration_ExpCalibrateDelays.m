classdef GIA_stabilizationDuration_ExpCalibrateDelays < ExperimentAWG
    % ExpCalibrateDelays Experiment for calibrating the delay between the
    % detection window to the acquisition - stabilization time of the GIA.
    % the code take in account the gate off delay
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    methods
        function obj = GIA_stabilizationDuration_ExpCalibrateDelays(FG)
            ...
            [~, obj.gateOffDelay] = pg.channelName2Delays('GIgate');
            obj.time = obj.gateOffDelay + 0:0.01:2;
    ...
    methods (Access = protected)        
        function prepare(obj)
            ...
            S = Sequence;
            S.addEvent(0.4,                         {'greenLaser', 'GIenable'});
            S.addEvent(0.25,                        {'greenLaser', 'GIenable', 'GIgate'});
            S.addEvent(abs(obj.gateOffDelay),            {'greenLaser', 'GIenable', 'GIgate'});                  % off delay. must added manualy
            S.addEvent(max(obj.time) - obj.gateOffDelay, ...
                                                    {'greenLaser', 'GIenable'},                 'time');
        ...
        function perform(obj)
            ...
            for n = randperm(length(obj.time))
                ...
                for trial = 1 : 5
                    ...
                    try
                        pg.changeSequence('time', 'duration', obj.time(n) - obj.gateOffDelay);
