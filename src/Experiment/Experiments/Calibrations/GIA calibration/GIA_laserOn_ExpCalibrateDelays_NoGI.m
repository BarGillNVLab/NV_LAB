classdef GIA_laserOn_ExpCalibrateDelays_NoGI < ExperimentAWG
    ...
    properties (Constant)
        NAME = 'GIA GIgate Calibration Delays No GI';
    end
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    methods
        function obj = GIA_laserOn_ExpCalibrateDelays_NoGI(FG)
...
%             obj.delays = 0.1:0.005:0.4;
    ...
    methods (Access = protected)        
        function prepare(obj)
            ...
            S = Sequence;
            S.addEvent(abs(max(obj.delays)),        {'greenLaser'},                 'delay1');
            S.addEvent(0,                           {'detector'},                   'delay2');
            S.addEvent(abs(min(obj.delays)),        {'greenLaser', 'detector'},     'delayBoth');
            S.addEvent(2,                           {'greenLaser'});
            S.addEvent(10,         '');           % NVs decay into mix state
            S.addEvent(0,                       'GIgate');                                      % enter gap to stand in the ADC maximum rate
