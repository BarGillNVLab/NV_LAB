classdef GIA_MWon_ExpCalibrateDelays < ExperimentAWG
    
    properties (Constant)
        NAME = 'GIA MW on calibration';
    end
    ...
    methods
        function obj = GIA_MWon_ExpCalibrateDelays()
...
%% Overridden from Experiment
    methods (Access = protected)
        ...
        function prepare(obj)
            ...
            S.addEvent(0,                           {'greenLaser'},                     'delayLaser1');
            S.addEvent(0,                  [{'greenLaser'}, MW(:)'],           'delayBoth');
            S.addEvent(0,                           {'greenLaser'},                     'delayLaser2');
            S.addEvent(obj.piTime,                  MW,                                 'delayMW');
            S.addEvent(0,                           '',                                 'initDelay');   % Last delay
...
%             AWG load and run:
            ...
            for t = randperm(length(obj.delays))
                ...
                for trial = 1 : 10
                    ...
                    try
%             S.addEvent(obj.piTime,                  [{'greenLaser'}, MW(:)'],           'delayBoth');
%             S.addEvent(0,                           {'greenLaser'},                     'delayLaser');
%             S.addEvent(0,                           MW,                                 'delayMW');
%             S.addEvent(obj.lastDelay,               '',                                 'lastDelay');   % Last delay
                        
                        if obj.delays(t) > 0    % MW on befor laser off
                            pg.changeSequence('delayLaser1', 'duration', max(obj.delays) - obj.delays(t));
                            if obj.delays(t) > obj.piTime
                                pg.changeSequence('delayBoth', 'duration', obj.piTime);
                                pg.changeSequence('delayLaser2', 'duration', obj.delays(t) - obj.piTime);
                                pg.changeSequence('delayMW', 'duration', 0);
                                pg.changeSequence('lastDelay', 'duration', maxLastDelay);
                            else
                                pg.changeSequence('delayBoth', 'duration', obj.delays(t));
                                pg.changeSequence('delayLaser2', 'duration', 0);
                                pg.changeSequence('delayMW', 'duration', obj.piTime - obj.delays(t));
                                pg.changeSequence('lastDelay', 'duration', maxLastDelay - (obj.piTime - obj.delays(t)));
                            end
                            pg.changeSequence('initDelay', 'duration', 0);
                            
                        else
                            pg.changeSequence('delayLaser1', 'duration', max(obj.delays));
                            pg.changeSequence('delayBoth', 'duration', 0);
                            pg.changeSequence('delayLaser2', 'duration', 0);
                            pg.changeSequence('initDelay', 'duration', -obj.delays(t));
                            pg.changeSequence('delayMW', 'duration', obj.piTime);
                            pg.changeSequence('lastDelay', 'duration', maxLastDelay - obj.piTime + obj.delays(t));
                        end
