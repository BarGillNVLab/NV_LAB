classdef ExpLaserOnOff < Experiment
    %ExpLaserOnOff Experiment for calibrating the delay times
    
    properties (Constant)
        NAME = 'LaserOnOff_signal';
    end
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    methods
        function obj = ExpLaserOnOff(FG)
            obj@Experiment(ExpLaserOnOff.NAME);
    ...
    methods
        ...
        function tEnd = get.tEnd(obj)
            if length(obj.channelDelay) == 1
                tEnd = obj.channelDelay * (length(obj.offTime) + 2) + sum(obj.offTime) + obj.detectionStartStopOffset;
...
%             checkFrequencyScalar(obj, newVal)
        ...
        function set.onTime(obj, newVal)	% newVal in microsec
            checkTimeVector(obj, newVal)
            % If we got here, then newVal is OK.
            obj.onTime = newVal;
            obj.changeFlag = true;
        ...
        function set.offTime(obj, newVal)	% newVal in microsec
            checkTimeVector(obj, newVal)
            % If we got here, then newVal is OK.
            obj.offTime = newVal;
            obj.changeFlag = true;
...
%                     end
                    S.addEventAtGivenTime(obj.channelDelay, obj.channelDelay, 'greenLaser');
                    stop_laser = obj.channelDelay*2;
                    for k = 1:length(obj.offTime)
                        start_laser = stop_laser + obj.offTime(k);
                        S.addEventAtGivenTime(stop_laser, obj.offTime(k), '');
                        S.addEventAtGivenTime(start_laser, obj.channelDelay, 'greenLaser');
                        stop_laser = start_laser + obj.channelDelay;
                    end
                end
