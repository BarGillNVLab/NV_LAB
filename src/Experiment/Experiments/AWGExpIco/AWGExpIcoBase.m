    ...
    methods (Access = protected)
        ...
        function LoadAWG(obj)
            ...
            for chan = 1:obj.nChannels
                ...
                f = @(t) A(chan) * ((prePulseTime(chan)                <=t & t<prePulseTime(chan)+durs(1))                 * signs(1) .* cos(w*t - phaseZero) + ...
                                    (prePulseTime(chan)+durs(1)        <=t & t<prePulseTime(chan)+durs(1)+durs(2))         * signs(2) .* cos(w*t + pi/2 - phaseZero) + ...
                                    (prePulseTime(chan)+durs(1)+durs(2)<=t & t<prePulseTime(chan)+durs(1)+durs(2)+durs(3)) * signs(3) .* cos(w*t - phaseZero));
