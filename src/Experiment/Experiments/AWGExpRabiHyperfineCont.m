classdef AWGExpRabiHyperfineCont < ExperimentAWG
    
    properties (Constant)
        NAME = 'AWG Rabi hyperfine';
    end
    ...
    methods
        function obj = AWGExpRabiHyperfineCont()
    ...
    methods (Access = protected)
        ...
        function prepare(obj)
            ...
            if ~obj.GIMeas
                ...
                if isa(spcm, 'PhotoDiodeNiDaqControlled')
                    S.addEvent(0.8,                     'greenLaser');                                  % enter gap to stand in the ADC maximum rate
                    S.addEvent(0,                       'GIgate');                                      % just to set spcm.detectionWithGI to true
                end
            else
        ...
        function LoadAWG(obj)
            baseWFtime = 0.2;
            baseWFtime = obj.RoundDurationByFrequencies(baseWFtime, [obj.frequency, obj.frequency - obj.freqSplit, obj.frequency + obj.freqSplit], 1);
            ...
            wL = 2*pi*(obj.frequency - obj.freqSplit);
            wR = 2*pi*(obj.frequency + obj.freqSplit);
            
            % the resolution of the time delay is the sample rate, so we add also aditional phase delay
            t0 = [0, obj.timeDelay];
            for chan = 1:obj.nChannels
                f = @(t) A(chan) * (cos(w*(t - t0(chan))) + cos(wL*(t - t0(chan))) + cos(wR*(t - t0(chan))));
