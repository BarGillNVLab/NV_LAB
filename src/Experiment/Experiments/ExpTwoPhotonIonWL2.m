classdef ExpTwoPhotonIonWL2 < Experiment
    ...
    properties
        ...
        waitTimesIon                % in us. Delay between ionization and detection.
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    methods
        function obj = ExpTwoPhotonIonWL2(FG, MWChannel)
            obj@Experiment(ExpTwoPhotonIonWL2.NAME);
    ...
    methods
        ...
        function set.waitTimesIon(obj, newVal)
            checkTimeVector(obj, newVal)
            % If we got here, then newVal is OK.
            obj.waitTimesIon = newVal;
            obj.changeFlag = true;
        end
    ...
    methods
        function totalParamNum = getTotalNumberOfParams(obj)
            totalParamNum = length(obj.laserPowers)*length(obj.waitTimesIon);
        end
    end
    ...
    methods (Access = protected)
        function prepare(obj)
            ...
            for k = 1:1+obj.doubleMeasurement
                ...
                S.addEvent(max(obj.waitTimesIon),      '',    'waitTimeIon')                                 % Delay for excited state decay
