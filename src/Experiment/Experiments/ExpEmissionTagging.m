classdef ExpEmissionTagging < Experiment
    % ExpEmissionTagging Emission tagging experiment
    
    properties (Constant)
        NAME = 'EmissionTagging';
    end
    ...
    properties
        ...
        % running);
        theta           % Initial state of experiment (angle of Bloch vector [rad] around y axis)
        piTime          % in us, according to Rabi experiment
        t_MW            % MW pulse time [us]
        frequency       % in MHz
        amplitude       % in dBm
        tau             % in us, laser duration
        nChannels
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    methods
        function obj = ExpEmissionTagging(FG, MWChannel)
            obj@Experiment(ExpEmissionTagging.NAME);
...
%             %%% 2d vector, e.g. frequency=[2870,2870], amplitude=[-10,-10]
    ...
    methods
        ...
        function set.theta(obj, newVal) % newVal is in MHz
%             checkTimeScalar(obj, newVal)
    ...
    methods (Access = protected)
       function prepare(obj)
            ...
            S.addEvent(obj.piTime * obj.theta / pi,        MW,                         'MW')               % MW
            S.addEvent(obj.smallDelay,        '')
            %S.addEvent(tau,           '',                         'lastDelay2');       % Last delay

            
            S.addEvent(max(obj.tau),...
                                            {'greenLaser', 'detector'}, 'laserDuration');                    % Laser and Detection

            S.addEvent(max(obj.tau) / 2,...
        ...
        function perform(obj)
            ...
            for t = tau_perm
                ...
                for trial = 1 : 10
                    ...
                    try
                        pg.changeSequence('laserDuration', 'duration', obj.tau(t));
                        if obj.constantTime
