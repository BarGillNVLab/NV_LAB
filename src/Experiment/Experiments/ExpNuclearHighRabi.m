classdef ExpNuclearHighRabi < Experiment
  ...
  % states
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    methods
        function obj = ExpNuclearHighRabi(FG, MWChannel)
            obj@Experiment(ExpNuclearHighRabi.NAME);
...
%            obj.freqGenName = {obj.getFgName(fg{1}), obj.getFgName(fg{2})};
            ...
            obj.tau_0 = 0.025;              % in us
              obj.tau_1 = 0.025;
                obj.tau_2 = 0.025;
            obj.piTime_up = 0.05;  % in us
             obj.piTime_down = 0.05; 
              obj.piTime_RF = 0.05; 
            obj.tau = [0.1,0.1];  
    ...
    methods
        ...
        function set.tau_2(obj, newVal)	% newVal in microsec
            checkTimeVector(obj, newVal)
            % If we got here, then newVal is OK.
            obj.tau_2 = newVal;
            obj.changeFlag = true;
        end
       function set.lasertime(obj, newVal)	% newVal in microsec
            checkTimeVector(obj, newVal)
            % If we got here, then newVal is OK.
            obj.lasertime = newVal;
            obj.changeFlag = true;
        end
...
%         end
        ...
        function set.piTime_up(obj, newVal)
            checkTimeScalar(obj, newVal)
            % If we got here, then newVal is OK.
            obj.piTime_up = newVal;
            obj.changeFlag = true;
        end
        
  
        function set.piTime_down(obj, newVal)
            checkTimeScalar(obj, newVal)
            % If we got here, then newVal is OK.
            obj.piTime_down = newVal;
            obj.changeFlag = true;
        end

        function set.piTime_RF(obj, newVal)
            checkTimeScalar(obj, newVal)
            % If we got here, then newVal is OK.
            obj.piTime_RF = newVal;
            obj.changeFlag = true;
        end
        ...
        function prepare(obj)
            ...
            S = Sequence;
             for k = 1:1+obj.doubleMeasurement
                S.addEvent(obj.piTime_down, 'MW');                           % MW in x
                
                % tau0 before rotation
                   S.addEvent(obj.tau_0, '', 'tau_0'); 
                    S.addEvent(obj.piTime_RF, 'MW2');% Delay
                % RF frequency to translate bw nuclear states 
                if obj.mixer % mixing 'MW' and 'MW2' (hardware connected through mixer)
                  S.addEvent(obj.lasertime,{'greenLaser','greenLaserOneOrTwo'}); 
                   S.addEvent(obj.tau_1, '', 'tau_1'); 
                   S.addEvent(obj.piTime_up, 'MW');
                     S.addEvent(obj.tau(end), 'MW2',...
                                'small pertubation');                       % MW2 in x + MW.phase
                else
                    S.addEvent(obj.tau_2, 'MW2', 'makeup for srs delay');
                    S.addEvent(obj.tau(end), {'MW2', 'MW'},...
                                'small pertubation');                       % MW2 in x + MW.phase
                    S.addEvent(obj.tau_2, 'MW2', 'makeup for srs delay');
                end
                % tau0 after rotation
                S.addEvent(obj.tau_2, '', 'tau_2');
                    
      
                S.addEvent(obj.piTime_up, 'MW');                       % MW in 
