classdef Exp2DESR < Experiment
    ...
    methods
        ...
        function obj = Exp2DESR(FGs, MWChannel)
            obj@Experiment(Exp2DESR.NAME);
    ...
    methods
        ...
        function set.amplitudes(obj, newVal) % newVal is in dBm
            checkAmplitudeVector(obj, newVal)
            % If we got here, then newVal is OK.
            obj.amplitudes = newVal;
            obj.changeFlag = true;
        end
    ...
    methods
        function totalParamNum = getTotalNumberOfParams(obj)
            ampSize = size(obj.amplitudes);
            totalParamNum = ampSize(2) * length(obj.frequency);
        end
    end
    ...
    methods (Access = protected)
        function prepare(obj)
            ...
            obj.mCurrentYAxisParam.value = obj.amplitudes;
        end
        ...
        function perform(obj)
            ...
            for k = 1:len
                ...
                for trial = 1 : 5
                    ...
                    try
                        ...
                        [m, ~] = ind2sub(size(obj.frequency),k);
                        fg.amplitude = obj.amplitudes(m);
            ...
            S1 = reshape(squeeze(obj.signal(1, :, 1:obj.currIter)), length(obj.amplitudes), len, []);
            S1sterr = reshape(squeeze(obj.sterr(1, :, 1:obj.currIter)), length(obj.amplitudes), len, []);
            S2 = reshape(squeeze(obj.signal(2, :, 1:obj.currIter)), length(obj.amplitudes), len, []);
            S2sterr = reshape(squeeze(obj.sterr(2, :, 1:obj.currIter)), length(obj.amplitudes), len, []);
            ...
            obj.signalParam.sterr = sterr;
             ...
             if isSingleMeasurement
                ...
                for i = 1:L
                    currentFreq = obj.frequency(i);
                    obj.signalParam.value(i, :) = interp1(currentFreq, value(i, :), obj.frequency, 'spline', NaN); % can change 'extrap' to be NaN or 0,
