classdef ExpESR_CS < Experiment
    %EXPESR_CS - Compressed Senssing ESR (electron spin resonance) Experiment
    %Multi MW 
    ...
    methods
        ...
        function obj = ExpESR_CS(FGs, MWChannel)
            obj@Experiment(ExpESR_CS.NAME);
            ...
            obj.singletDelay = 1;
            obj.MWChannel = {'MW' , 'MW2', 'MW3'}; %galya
    ...
    methods
        ...
        function set.window_start(obj, newVal)
            checkFrequencyScalar(obj, newVal)
            % If we got here, then newVal is OK.
            obj.window_start = newVal;
            obj.changeFlag = true;
        end
        
         function set.window_end(obj, newVal)
            checkFrequencyScalar(obj, newVal)
            % If we got here, then newVal is OK.
            obj.window_end = newVal;
            obj.changeFlag = true;
         end
        
         function set.freq_range(obj, newVal)	% newVal in microsec
            checkFrequencyVector(obj, newVal)
            % If we got here, then newVal is OK.
            obj.freq_range = newVal;
            obj.changeFlag = true;
        end
end
    ...
    methods
        ...
        %         end
        function ESR_CS = SaveExperiment(obj, fileName)
            ...
            for k = 1:length(names)
                command = sprintf('ESR_CS.%s = obj.%s;',names{k},names{k});
                eval(command);
            end
            ESR_CS.date = datestr(now,'yyyy-mm-dd_HH-MM-SS');
            eval(sprintf('save(''%s'',''ESR_CS'');',fileName));
        end
    end
                ...
                case 'CW'
                    ...
                    switch obj.nChannels
                        case 1
                            ...
                            S.addEvent(obj.detectionDuration,        {MWChannel{1}, 'greenLaser', 'detector'});
                        case 2
                            S.addEvent(obj.laserInitializationDuration,     {MWChannel{1}, MWChannel{2}, 'greenLaser'});
                            S.addEvent(obj.detectionDuration,        {MWChannel{1}, MWChannel{2}, 'greenLaser', 'detector'});
                        case 3
                            obj.WindFreakChannels = [1, 1];
                            S.addEvent(obj.laserInitializationDuration,     {MWChannel{1}, MWChannel{2}, MWChannel{3}, 'greenLaser'});
                            S.addEvent(obj.detectionDuration,        {MWChannel{1}, MWChannel{2}, MWChannel{3}, 'greenLaser', 'detector'});
                        otherwise
...
%             if obj.nChannels == 1
%             fg = getObjByName(obj.freqGenName{1});
%             end
...
%             isSingleMeasurement = (isempty(obj.mirrorSweepAround) || obj.nChannels > 1);
            ...
            sterr = zeros(1, n);
            contrast = 0;
            obj.CS = spike_adaptive_l1homotopy_s2(obj.freq_range,obj.nChannels,contrast,obj.window_start,obj.window_end);%,obj.Raster); %Galya
            frequency = [];
...
%                 end
                ...
                for trial = 1 : 5
                    ...
                    try
%                         if obj.nChannels == 1
                            obj.setMultipleFGFrequencies(freq')
%                         else % This will run both channels at the same time
%                             if obj.equalChannels 
%                                 obj.setMultipleFGFrequencies([freq(1), freq(2)]);
%                                 fg.phase = [0, obj.calculatePhase(obj.frequency(i))];
%                             else
%                                 obj.setMultipleFGFrequencies([freq(1), freq(2)]); %galya- 3 freqs??
%                             end
%                         end
...
%                         end
                ...
                obj.signal(:, k, obj.currIter) = sig;
                obj.sterr(:, k, obj.currIter) = sterr;
%                 obj.signal= [sig(1); sig(2)]; %galya , sterr is reference? whats the dimentions?
                obj.CS = obj.CS.process_data(sig);
                disp(k)
            end
            % Set parameter, for saving
%             obj.mCurrentXAxisParam.value = frequency;
            ...
            if length(data) == obj.detectionPeriodsPerRepeat*obj.repeats*obj.getTotalNumberOfParams %if true we're proccessing data at the end of the average
                for k = 1:len
                    [sig(1:2), sterr(1:2)] = obj.processData(data(obj.detectionPeriodsPerRepeat*obj.repeats*(k-1)+(1:obj.detectionPeriodsPerRepeat*obj.repeats)));
%                     if ~isSingleMeasurement % run another sweep with the same source
...
%                     end
            ...
            S1 = squeeze(obj.signal(1, :, 1:obj.currIter));  %galya, what about this??
