classdef Exp2DEchoNpulseWithACMagnetic1Tau < Experiment
    %EXPECHO Echo experiment With AC Magnetic faild

    properties (Constant)
        NAME = 'ExpEchoNpulseWithACMagnetic1Tau';
        XY4_PULSES = {'X', 'Y', 'Y', 'X'};
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    methods
        function obj = Exp2DEchoNpulseWithACMagnetic1Tau(FG, MWChannel)
            obj@Experiment(ExpEchoNpulseWithACMagnetic1Tau.NAME);
            ...
            obj.pulsesMode="xy-n";          % CPMG-n ot XY-n
    ...
    methods
        ...
        function twoDparm = twoDparmMaker(obj)
            n=0;
            for i=1:length(obj.tau)
                for j=1:length(obj.cycles)
                    n=n+1;
                    twoDparm={[obj.tau(i) ,j ]};
                end
            end
        end
    ...
    methods (Access = protected)
        function prepare(obj)
            ...
            obj.loadRigolAWG;
            obj.setRigolNPulse(obj.cycles(1))
            % Set parameter, for saving
            obj.mCurrentXAxisParam.value = obj.cycles*obj.tau(end);
        ...
        function loadRigolAWG(obj)
            ...
            obj.ACsource.setFreq(0.5/obj.tau(end));  
            obj.ACsource.setPeriod(obj.tau(end)*2);
        ...
        function setRigolNPulse(obj,cycle)
        obj.ACsource.burstNcycle(ceil(cycle/2));          
        end
        function perform(obj)
            ...
            %%% Run - Go over all N cycles, in random order
            
            twoDparm=obj.twoDparmMaker;
            for n = randperm(length(twoDparm))
                ...
                for trial = 1 : 5
                    ...
                    try
                        Ctau = cell2mat(twoDparm{n}(1));
                         
                        pg.setSequence(obj.privateSequences{cell2mat(twoDparm{n}(2))});
                        obj.setRigolNPulse(obj.cycles(cell2mat(twoDparm{n}(2))))
                        pg.changeSequence('tau', 'duration',Ctau);
                        
                        if obj.ACamplitude >0
                        obj.ACsource.burstNcycle(round(2*obj.cycles*Ctau*obj.AcFrequency+5)); %5*obj.tau(t)*obj.AcFrequency
                        end
