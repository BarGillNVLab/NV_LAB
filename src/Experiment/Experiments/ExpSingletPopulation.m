classdef ExpSingletPopulation < Experiment
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    methods
        function obj = ExpSingletPopulation(FG, MWChannel)
            obj@Experiment(ExpSingletPopulation.NAME);
            ...
            obj.PopDuration = 0.1;
            obj.waitTimeExcited = 0.2; %0.03;
            obj.waitTimeSinglet = 0.3; %0.5;
...
%             obj.isPlotAlternateAvailable = true;
            ...
            obj.mCurrentYAxisParam = ExpParamDoubleVector('Population Pulse Duration', [], [], StringHelper.MICROSEC, obj.NAME);
%             obj.signalParam = ExpResultDoubleVector('FL', [], [], 'Normalized', obj.NAME);
    ...
    methods
        function totalParamNum = getTotalNumberOfParams(obj)
            totalParamNum = length(obj.laserPowers) *  length(obj.popDurations);
        end
    end
...
%                 if k==1
                    S.addEvent(obj.detectionDuration,          {'IR','detector'})                  % Ionization from Singlet state
                    S.addEvent(0.5,          {'IR'})
%                 else
%                     S.addEvent(obj.detectionDuration,          {'detector'})                  % Ionization from Singlet state
%                 end
                
                S.addEvent(1,          '')                                 % Delay for singlet state decay
...
%                     {'greenLaser', 'detector'});                                            % Detection
            ...
            obj.mCurrentYAxisParam.value = obj.popDurations;
        ...
        function perform(obj)
            ...
            for t = randperm(length(obj.laserPowers))
                ...
                for p = randperm(length(obj.popDurations))
                    ...
                    if obj.currIter <= size(obj.signal, 3) && ...
                            sum(obj.signal(:, (t-1)*length(obj.popDurations)+p, obj.currIter)) ~= 0
                        continue
                    end
                    %                     if obj.currIter <= size(obj.signal, 3) && ...
           ...
           S1 = squeeze(obj.signal(1, :, 1:obj.currIter));
            ...
            value = reshape(value,length(obj.popDurations),length(obj.laserPowers));
            sterr = reshape(sterr,length(obj.popDurations),length(obj.laserPowers));
