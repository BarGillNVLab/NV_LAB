classdef AWGsensingSingleDriveBscan < ExperimentAWG
    ...
    methods
        function obj = AWGsensingSingleDriveBscan()
            obj@ExperimentAWG(AWGsensingSingleDriveBscan.NAME);
...
%% Overridden from Experiment
    methods (Access = protected)
        ...
        function LoadAWG(obj)
            ...
            for i = 1:N
                for chan = 1:obj.nChannels
                    f = @(t) A1(chan) *  obj.ampRatios(i) * cos(w*t - phase_A1(chan)) + g(chan)*cos(ws*t - phase_s(chan) + pi/2);
                    constStepIndex(chan, i) = obj.AWG.AddWaveformByFunction(f, baseWFtime, 0);
                    f = @(t) (A1(chan) * obj.ampRatios(i) * cos(w*t - phase_A1(chan)) + g(chan) * cos(ws*t - phase_s(chan) + pi/2)) .* (0<=t & t<residue);
