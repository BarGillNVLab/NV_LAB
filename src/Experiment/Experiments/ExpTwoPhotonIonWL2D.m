classdef ExpTwoPhotonIonWL2D < Experiment
    ...
    properties
        ...
        ionDurations                % in us. Ionization of the singlet state.
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    methods
        function obj = ExpTwoPhotonIonWL2D(FG, MWChannel)
...
%             obj.signalParam = ExpResultDoubleVector('FL', [], [], 'Normalized', obj.NAME);
    ...
    methods
        ...
        function set.ionDurations(obj, newVal)
            checkDetectionDuration(obj, newVal)
            % If we got here, then newVal is OK.
            obj.ionDurations = newVal;
            obj.changeFlag = true;
        end
    ...
    methods
        function totalParamNum = getTotalNumberOfParams(obj)
            totalParamNum = length(obj.laserPowers)*length(obj.ionDurations);
        end
    end
    ...
    methods (Access = protected)
       function prepare(obj)
            ...
            for k = 1:1+obj.doubleMeasurement
                ...
                if k==1
                    S.addEvent(max(obj.ionDurations),          {'whiteLaser'}, 'ionDuration')                  % Ionization from Singlet state 
                else
                    %                   S.addEvent(obj.IonDuration,          'greenLaser','greenLaserOneOrTwo')                  % Ionization from Singlet state
                    S.addEvent(max(obj.ionDurations),          '', 'ionDuration')                  % Ionization from Singlet state
...
%             maxLastDelay = Experiment.DEFAULT_LAST_DELAY + max(obj.tau);
            ...
            for t = randperm(length(obj.laserPowers))
                ...
                for d = randperm(length(obj.ionDurations))
                    if obj.currIter <= size(obj.signal, 3) && ...
                            sum(obj.signal(:, (t-1)*length(obj.ionDurations)+d, obj.currIter)) ~= 0
                        continue
                    end
                    success = false;
                    
                    ionDuration = obj.ionDurations(d);
                    ...
                    for trial = 1 : 10
                        ...
                        try
                            ...
                            pg.changeSequence('ionDuration', 'duration', ionDuration);
                            
                            data = obj.getRawData(pg, spcm);
                            [sig, sterr] = obj.processData(data);
                            
                            obj.signal(:, (t-1)*length(obj.ionDurations)+d, obj.currIter) = sig;
                            obj.sterr(:, (t-1)*length(obj.ionDurations)+d, obj.currIter) = sterr;
            ...
            [valueVector, sterrVector] = getRatioDistributionValues(obj, S1, S2, S1sterr, S2sterr);
            value = reshape(valueVector, length(obj.ionDurations), length(obj.laserPowers));
            sterr = reshape(sterrVector, length(obj.ionDurations), length(obj.laserPowers));
            ...
            if obj.doubleMeasurement
                ...
                [valueVector, sterrVector] = getRatioDistributionValues(obj, S3, S4, S3sterr, S4sterr);
                value = reshape(valueVector, length(obj.ionDurations), length(obj.laserPowers));
                sterr = reshape(sterrVector, length(obj.ionDurations), length(obj.laserPowers));
