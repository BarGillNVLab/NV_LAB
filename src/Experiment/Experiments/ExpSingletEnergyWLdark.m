classdef ExpSingletEnergyWLdark < Experiment
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    methods
        function obj = ExpSingletEnergyWLdark(FG, MWChannel)
            obj@Experiment(ExpSingletEnergyWLdark.NAME);
...
%             obj.signalParam = ExpResultDoubleVector('FL', [], [], 'Normalized', obj.NAME);
    ...
    methods (Access = protected)
       function prepare(obj)
            ...
            obj.detectionPeriodsPerRepeat = 3 * (1 + double(obj.doubleMeasurement));
...
%                 S.addEvent(obj.PopDuration,              {'greenLaser','greenLaserOneOrTwo'})                       % Populating the singlet
                S.addEvent(obj.PopDuration,              {'greenLaser'})                       % Populating the singlet
...
%                     S.addEvent(obj.IonDuration,          'greenLaser')                  % Ionization from Singlet state
                ...
                S.addEvent(obj.waitTimeSinglet,          'detector')
                S.addEvent(obj.waitTimeSinglet,          '')
        ...
        function perform(obj)
            ...
            laser = getObjByName('White Laser');
            [~, laserParts] = laser.getContollableParts();
            % Now assumes laser changing the first laser part:
            laserPart = laserParts{1}; 
...
%                         pg.changeSequence('initDuration', 'duration', initDuration);
            ...
            S1dark = squeeze(obj.signal(1, :, 1:obj.currIter));
%             S1darksterr = squeeze(obj.sterr(1, :, 1:obj.currIter));
            S1 = squeeze(obj.signal(2, :, 1:obj.currIter));
            S1sterr = squeeze(obj.sterr(2, :, 1:obj.currIter));
            S2 = squeeze(obj.signal(3, :, 1:obj.currIter));
            S2sterr = squeeze(obj.sterr(3, :, 1:obj.currIter));
            
            [value, sterr] = getRatioDistributionValues(obj, (S1-S1dark), (S2-S1dark), S1sterr, S2sterr);
            obj.signalParam.value = value;
            obj.signalParam.sterr = sterr;  
            
            if obj.doubleMeasurement
                S3dark = squeeze(obj.signal(4, :, 1:obj.currIter));
%                 S3darksterr = squeeze(obj.sterr(4, :, 1:obj.currIter));
                S3 = squeeze(obj.signal(5, :, 1:obj.currIter));
                S3sterr = squeeze(obj.sterr(5, :, 1:obj.currIter));
                S4 = squeeze(obj.signal(6, :, 1:obj.currIter));
                S4sterr = squeeze(obj.sterr(6, :, 1:obj.currIter));
                
                [value, sterr] = getRatioDistributionValues(obj, (S3-S3dark), (S4-S3dark), S3sterr, S4sterr);
