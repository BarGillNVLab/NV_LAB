classdef ExpDoubleRabi_Nuclear < Experiment
    ...
    properties
        ...
        cooling_delay       % in us. Delay between sequences to cool off from MW
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    methods
        function obj = ExpDoubleRabi_Nuclear(FGs)
            obj@Experiment(ExpDoubleRabi_Nuclear.NAME);
            ...
            obj.repeats = 10000;
            obj.averages = 1000;
            obj.MWChannel = {'MW', 'MW2'};  % mw channels to use, set by default to all 3 MW.
            ...
            obj.singletDelay = 0.001;
            obj.readDelay = 0.001;
            obj.cooling_delay = 0.001;
            obj.startReadPairs = {[1, 0], [1, 1], [1, -1]};
            obj.MWChannel_detuned ={};
    ...
    methods
        ...
        function set.cooling_delay(obj, newVal)
            checkTimeScalar(obj, newVal)
            % If we got here, then newVal is OK.
            obj.cooling_delay = newVal;
            obj.changeFlag = true;
        end
    end
...
%                 end
                switch startFrom
                    case 0
                        % Nothing
                    case 1
%                         obj.setMultipleFGFrequencies([2914, obj.frequency(2), obj.frequency(3)]);
                        S.addEvent(obj.singletDelay,        '',                         'singletDelay');         % Singlet Delay
                        S.addEvent(obj.piTime(1),  obj.MWChannel{2},                    'initPulse');   % MW
                    case -1
%                         obj.setMultipleFGFrequencies([2824, obj.frequency(2), obj.frequency(3)]);
                        S.addEvent(obj.singletDelay,        '',                         'singletDelay');
                        S.addEvent(obj.piTime(2),  obj.MWChannel{2},                   'initPulse');   % MW
                end
                % S.addEvent(0.2,           '',     'delay');
                S.addEvent(obj.tau(end),        obj.MWChannel_detuned,                         'tau');         % MW for +1/-1 transition
                % S.addEvent(0.2,           '',     'delay');
                switch readFrom
                    case 0
                        S.addEvent(obj.piTime(1),  '',                    'equalPiDelay');     % add delay to match pi-pulse
                    case 1
%                         obj.setMultipleFGFrequencies([2914, obj.frequency(2), obj.frequency(3)]);
                        S.addEvent(obj.piTime(1),  obj.MWChannel{2},                    'readPulse');   % MW
                    case -1
%                         obj.setMultipleFGFrequencies([2824, obj.frequency(2), obj.frequency(3)]);
               ...
               if obj.cooling_delay > 0.001
                    S.addEvent(obj.cooling_delay,                       '',                         'coolOffDelay');
                end
