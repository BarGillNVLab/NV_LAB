classdef AWGExpIcoDynamicalBase < ExperimentAWG
    %EXPECHO Echo experiment

    properties (Constant)
        NAME = 'AWG Icosahedral Dynamical Base';
    end
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    methods
        function obj = AWGExpIcoDynamicalBase()
            ...
            obj.mormSegmentDuration = 0.3;        % us
        ...
        function LoadAWG(obj)
            ...
            for chan = 1:obj.nChannels
                ...
                f = @(t) A(chan) * (prePulseTime(chan)  <=t & t<prePulseTime(chan) + obj.halfPiTime)                                     .* cos(w*t - phaseZero) + ...
                                    (prePulseTime(chan) + obj.halfPiTime <=t & t<prePulseTime(chan)+durs(1))                 * signs(1) .* cos(w*t - phaseZero) + ...
                                    (prePulseTime(chan)+durs(1)+obj.halfPiTime          <=t & t<prePulseTime(chan)+durs(1)+durs(2)+ obj.halfPiTime)         * signs(2) .* cos(w*t + pi/2 - phaseZero) + ...
                                    (prePulseTime(chan)+durs(1)+durs(2)+obj.halfPiTime  <=t & t<prePulseTime(chan)+durs(1)+durs(2)+durs(3)+obj.halfPiTime) * signs(3) .* cos(w*t - phaseZero);
            ...
            for chan = 1:obj.nChannels
                ...
                for i = 1:length(obj.pulseAngles)
                    ...
                    if i==(length(obj.pulseAngles)+1)/2 %Pi pulse for dynamical decoupling
                        ...
                        total_duration = total_duration + obj.segmentDurs(i+1) + obj.piTime;
                        sumMainFunction = @(t) sumMainFunction(t) + f(t);
                    else
                    total_duration = total_duration + obj.segmentDurs(i+1);
                    end 
