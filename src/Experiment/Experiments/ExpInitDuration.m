classdef ExpInitDuration < Experiment
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    methods
        function obj = ExpInitDuration(FG, MWChannel)
...
%             %%% 2d vector, e.g. frequency=[2870,2870], amplitude=[-10,-10]
    ...
    methods (Access = protected)
       function prepare(obj)
            ...
            S = Sequence;
            S.addEvent(obj.MWtime + min(obj.tau),           [{'greenLaser'}', MW(:)'])                                             % MW const
            S.addEvent(0,                                   [{'greenLaser'}', MW(:)'],                  'MW1')                     % MW 1
            S.addEvent(obj.detectionDuration,               [{'greenLaser', 'detector'}', MW(:)'],      'detection1')              % detection with MW
            S.addEvent(abs(min(obj.tau))-obj.detectionDuration,...
                                                            [{'greenLaser'}', MW(:)'],                  'MW2')                     % MW 2
            S.addEvent(0,                                   {'greenLaser'},                             'laser1');                 % laser 1
            S.addEvent(0,                                   {'greenLaser', 'detector'},                 'detection2');             % Detection without MW
            S.addEvent(0,                                   {'greenLaser'},                             'laser2');                 % laser 2
            ...
            S.addEvent(1,                                   'greenLaser');                                  % 1usec delay to ensure full overlap of ref detection and laser, Yachel 27.02.22
        ...
        function perform(obj)
            ...
            for t = randperm(length(obj.tau))
                ...
                for trial = 1 : 10
                    ...
                    try
                        if obj.tau(t) < -obj.detectionDuration
                            pg.changeSequence('MW1', 'duration', abs(min(obj.tau)) + obj.tau(t));
                            pg.changeSequence('detection1', 'duration', obj.detectionDuration);
                            pg.changeSequence('MW2', 'duration', abs(obj.tau(t)) - obj.detectionDuration);
                            
                            pg.changeSequence('laser1', 'duration', 0);
                            pg.changeSequence('detection2', 'duration', 0);
                            pg.changeSequence('laser2', 'duration', max(obj.tau));
                        elseif obj.tau(t) < 0
                            pg.changeSequence('MW1', 'duration', abs(min(obj.tau)) + obj.tau(t));
                            pg.changeSequence('detection1', 'duration', abs(obj.tau(t)));
                            pg.changeSequence('MW2', 'duration', 0);
                            
                            pg.changeSequence('laser1', 'duration', 0);
                            pg.changeSequence('detection2', 'duration', obj.detectionDuration - abs(obj.tau(t)));
                            pg.changeSequence('laser2', 'duration', max(obj.tau) - obj.detectionDuration - obj.tau(t));
                        else
                            pg.changeSequence('MW1', 'duration', abs(min(obj.tau)));
                            pg.changeSequence('detection1', 'duration', 0);
                            pg.changeSequence('MW2', 'duration', 0);
                            
                            pg.changeSequence('laser1', 'duration', obj.tau(t));
                            pg.changeSequence('detection2', 'duration', obj.detectionDuration);
                            pg.changeSequence('laser2', 'duration', max(obj.tau) - obj.detectionDuration - obj.tau(t));
                        end
