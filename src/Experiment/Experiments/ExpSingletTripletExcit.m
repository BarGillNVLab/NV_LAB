classdef ExpSingletTripletExcit < Experiment
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    methods
        function obj = ExpSingletTripletExcit(FG, MWChannel)
            obj@Experiment(ExpSingletTripletExcit.NAME);
            ...
            obj.detectionLaserPower = 0.01; % in whatever units the setup works with
            
            obj.constantTime = 0;
            obj.doubleMeasurement = true;   % logical
            
            obj.detectionDuration = 0.1; %0.25; % detection window, in us
            obj.referenceDetectionDuration = 5; % in us. Detection duration of the reference read
            obj.laserInitializationDuration = 300; % laser initialization in pulsed experiments
            
            obj.PopDuration = 0.4; %1;
            obj.IonDuration = 0.1; %1;
...
%             obj.signalParam = ExpResultDoubleVector('FL', [], [], 'Normalized', obj.NAME);
            obj.signalParam = ExpResultDoubleVector('FL', [], [], 'Normalized', obj.NAME, 'with \pi');
    ...
    methods (Access = protected)
       function prepare(obj)
            ...
            for k = 1:1+obj.doubleMeasurement
                for j = 1:2
                    S.addEvent(0.5,          '')                                 % Delay for singlet state decay
                    if k==1
                    ...
                    if j ==1
                        ...
                        S.addEvent(obj.PopDuration,              '' ,'populationDuration')                       % Populating the singlet
                    end
                    S.addEvent(obj.waitTimeExcited,          '')
                    switch obj.laserSource
                        case 'White Laser'
                            S.addEvent(obj.IonDuration,          {'whiteLaser','detector'} )                  % Ionization from Singlet state
                        case 'IR Laser'
                            S.addEvent(obj.IonDuration,          {'IR','detector'})                  % Ionization from Singlet state
                            S.addEvent(0.5,          {'IR'})
                        case 'Green Laser'
...
%                         pg.changeSequence('initDuration', 'duration', initDuration);
        ...
        function dataParam = alternateSignal(obj)
            ...
            pi = obj.signalParam.value;
            noPi = obj.signalParam2.value;
