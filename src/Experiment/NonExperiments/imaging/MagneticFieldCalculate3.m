function [B] = MagneticFieldCalculate3(dips, I_helm, theta,theta_x,theta_y)
...
% convertion of current to magnetic field for helmholtz
...
[m, n, ~] = size(dips);
B_meas_NV = zeros(m, n, 4);

for i = 1:4
    B_meas_NV(:,:,i) = ((dips(:,:,4+i) - dips(:,:,5-i)));
end
B_meas_NV = B_meas_NV/2 * MHz_TO_uT;                            % convert MHz to uT on each NV axis
B_meas_NV = B_meas_NV(:,:,order) .* reshape(signs,1,1,4);       % reorder and sings due to the expected field
B = zeros(m, n, 3);
for i = 1:m
    for j = 1:n
        B(i,j,:) = reshape(B_meas_NV(i,j,:),1,4) * NVtoXYZ * RotateBack * RotateBack_X* RotateBack_Y;
    end
end
B_meas_NV = B_meas_NV(order) .* reshape(signs,1,1,4);       % reorder and sings due to the expected field
% B_orientation=B_meas_NV;
