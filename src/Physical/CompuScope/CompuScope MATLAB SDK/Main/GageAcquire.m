...
[ret, sn] = CsMl_GetSerialNumber(handle);
disp(sn);
...
for i=1:sysinfo.TriggerCount
   [ret, trigger] = CsMl_QueryTrigger(handle, i);
end
...
[retval, channelinfo] = CsMl_QueryChannel(handle, 1);
% chan.InputRange = 200;
% CsMl_ConfigureChannel(handle, chan)
...
for i = 1:ChannelSkip:sysinfo.ChannelCount
    ...
    filename = sprintf('Acquire_CH%d.txt', i);
    ...
    info.SampleBits = acqInfo.SampleBits;
