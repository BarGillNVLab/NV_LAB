% GageSpecialMultipleRecord.m sample progam
...
% if you want timestamp data, transfer it before changing the segment size
% and depth
...
%transfer.Segment = 1;
%[ret, tsdata, tickfr] = CsMl_Transfer(handle, transfer, 1);

% change the segment size and depth and commit it, but dont' do a capture.
% This should allow us to transfer all records for each channel at once.
% We'll have to include the tail ( 64 bytes (32 samples for 14 bit boards)
% divided by the number of active channels) in the segment size and depth
acqInfo.SegmentSize = (acqInfo.SegmentSize + (64 / acqInfo.Mode / sysinfo.SampleSize)) * acqInfo.SegmentCount;
acqInfo.Depth = (acqInfo.Depth + (64 / acqInfo.Mode / sysinfo.SampleSize)) * acqInfo.SegmentCount;
...
% now transfer the whole thing at once
...
for channel = 1:ChannelSkip:sysinfo.ChannelCount
    ...
    for i = 1:acqInfo.SegmentCount
        transfer.Segment = i;
        % NOTE: we're transferring the data as raw values
...
% % %             length = size(data, 2);
% % %         end;     
% NOTE: For this example we're NOT writing a header and we're transferring 
% the data as binary 16 bit integers so there's no conversion to volts.
% This should speed up the filesaving from 40 seconds to about 2 seconds.
% The data is saved as the original segment size (i.e. 256 samples)
% followed by a tail which is 32 samples / number of active channels long.
% This pattern repeats for the number of segments that were captured.
...
% % % %        CsMl_SaveFile(filename, data, info, 1);
% % %         fid = fopen(filename, 'w');
% % %         if ( sysinfo.SampleSize == 1 )
% % %             count = fwrite(fid, data, 'uchar');
% % %         else
% % %             count = fwrite(fid, data, 'integer*2');
% % %         end
% % %         fclose(fid);
