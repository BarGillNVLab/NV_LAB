classdef ClassANC300 < ClassStage
    %ClassANC300 Used to control Attocube's ANC stage
    
    properties (Constant)
        NAME = 'Stage (Coarse) - ANC300'
        VALID_AXES = 'xyz';
        UNITS = 'um'
        X = 1; 
        Y = 2; 
        ...
        V_MAX = 150;    % float. in Volts
        V_MIN = 0;      % float. in Volts 
        F_MAX = 10e3;   % int. in Hz

        UP = 'u';
        DOWN = 'd';

        % Ethernet connection
        TELNET_0 = 7230;        % standard console
        TELNET_1 = 7231;        % LUA console
        IP = '132.64.56.63';   % ANC300 current ip
        PASSWORD = '123456';    % ANC300 access password

        TIMEOUT = 60e-3;       % in ms

        % init parameters x y z <=> 1 2 3
        AXES_FREQS = [1500 1500 1000];
        AXES_VOLTS = [40 40 40];
        AXES_SET_FUNCS = ["setm %d stp" "setf %d %d" "setv %d %d"];
        XYZ = [1 2 3];
        ...
        STEP_TIME = 100;    % mus. step time of the controller
    end
    
    properties
        deviceHandle = -1;  % Default value. Means stage is disconnected
        debug = 1;          % print ANC output
        
        % Default values
        stageOffset = [0 0 0];
        posRangeLimit = ClassANC300.POSITIVE_HARD_LIMITS;
        negRangeLimit = ClassANC300.NEGATIVE_HARD_LIMITS;
        posSoftRangeLimit = ClassANC300.POSITIVE_HARD_LIMITS;
        negSoftRangeLimit = ClassANC300.NEGATIVE_HARD_LIMITS;
    ...
    methods (Static, Access = public)
        ...
        function obj = create(stageStruct)
            ...
            missingField = FactoryHelper.usualChecks(stageStruct, ClassANC300.NEEDED_FIELDS);
...
%             niDaqChannel = stageStruct.niDaqChannel;
            removeObjIfExists(ClassANC300.NAME);
            obj = ClassANC300;       
        end
...
%         end
    ...
    methods (Access = private)
        function obj = ClassANC300()
            name = ClassANC300.NAME;
            availAxes = ClassANC300.VALID_AXES;
...
%         end
        ...
        function step = get.stepSize(obj)
            step = obj.voltage * obj.freq;
        end
        
        function v = get.curVel(obj)
            v = obj.stepSize / obj.STEP_TIME;
        end
