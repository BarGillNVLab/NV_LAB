classdef ViewPlacementControl < GuiComponent & EventListener
    ...
    methods
        function obj = ViewPlacementControl(parent, controller)
            ...
            %%%% panel init %%%%
            panelMain = uix.Panel('Parent', parent.component, 'Title', '', 'Padding', 5);
            ...
            gridLeftHeightTotal = heightGridTotalNoSpacing + 3 * 3;
        ...
        function refresh(obj)
            obj.edtSteps.String = StringHelper.formatNumber(obj.helm.stepSizeD);
            currentAngles = obj.helm.Dangles;   
            for i = 1:3
                obj.tvCurB(i).String = StringHelper.formatNumber(currentAngles(i));
            end
            obj.btnMoveToBlue.Enable = 'on';
        end
        
        function edtStepSizeCallback(obj)
            obj.helm.stepSizeD = str2double(obj.edtSteps.String);
        end
        
        function btnMoveCallback(obj, index, trueForLeftFalseForRight)
            step = BooleanHelper.ifTrueElse(trueForLeftFalseForRight,-1,1)*obj.helm.stepSizeD;
            obj.helm.(obj.helm.DIAMOND_AXES{index}) =  obj.helm.(obj.helm.DIAMOND_AXES{index}) + step;
            obj.refresh;
        end
        ...
        function btnMoveToBlueCallback(obj)
            ...
            for i = 1:3
                ang = obj.edtCurBValue(i);
                if ang ~= inf
                    obj.helm.(obj.helm.DIAMOND_AXES{i}) = ang;
                    obj.clearEdtCurB(i);
                end
            end
            isBeingGrayed = false;
        ...
        function tvCurBCallback(obj, index)
            ...
            obj.refresh
            obj.tvCurB(index).String = obj.helm.(obj.helm.DIAMOND_AXES{index});
        end
        ...
        function checkEdtCurBValue(obj, index)
            ...
            if ValidationHelper.isStringValueANumber(obj.edtCurB(index).String)
            ...
            elseif isempty(obj.edtCurB(index).String)
                ...
                EventStation.anonymousWarning('Value must be a number.')
            end
