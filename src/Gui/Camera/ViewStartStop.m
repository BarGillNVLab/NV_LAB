classdef ViewStartStop < GuiComponent
...
%         initialroi             % updates only when acquiring a new image
    ...
    methods
        function obj = ViewStartStop(parent, controller)
            ...
            bottom = uix.HBox('Parent', vboxMain, 'Spacing', 5, 'Padding', 16);
            obj.contchx = uicontrol(obj.PROP_CHECKBOX{:}, 'Parent',bottom, 'String', 'continuous');
            uix.Empty('Parent', bottom)
            uicontrol(obj.PROP_BUTTON{:}, ...
                'Parent', bottom, ...
                'String', 'Figure', ...
                'Callback', @obj.btnOpenInFigureCallback);
            bottom.Widths = [-1 10 70];
            vboxMain.Heights = [50 50 50];
...
%             obj.initialroi = cameracpture.mcameraimageparams.roi;
        ...
        function btnOpenInFigureCallback(obj, ~, ~) %#ok<INUSD>
            ...
            isVisible = true;
            cameradisplay.copyToFigure(isVisible);
        end
